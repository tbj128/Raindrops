...
public class AnimDiamonds {
     ...
     */
    static AnimDiamonds createFlake(float xRange, float yRange, Bitmap originalBitmap) {
        AnimDiamonds flake = new AnimDiamonds();
        // Size each flake with a width between 5 and 55 and a proportional height
        flake.width = (int)(12 + (float)Math.random() * 16);
        ...
        flake.y = (yRange * 2 / 3) + (-200 + (float)Math.random() * 400);

        flake.alpha = 500 + (float) Math.random() * 50;
        
        // Each flake travels at 50-200 pixels per second
        flake.speed = 40 + (float) Math.random() * 60;
        flake.xspeed = -100 + (float) Math.random() * 200;

        // Flakes start at -90 to 90 degrees rotation, and rotate between -45 and 45
        // degrees per second
        flake.rotation = (float) Math.random() * 180 - 90;
        flake.rotationSpeed = (float) Math.random() * 225 - 90;
